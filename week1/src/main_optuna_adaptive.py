...
def optimize_model(trial):
    ...
    rho = trial.suggest_float('rho', 0, 1)  # Rho Gaussian Modelling between 0 and 1
    ...
    #opening_kernel = trial.suggest_categorical('opening_kernel', [0, 3, 5, 7, 9, 11, 13,15])
    #closing_kernel = trial.suggest_categorical('closing_kernel', [0, 3, 5, 7, 9, 11, 13,15])

    # Print the parameters being tried
    print(f"Trying parameters: alpha={alpha}, rho={rho}, area_threshold={area_threshold}, aspect_ratio_threshold={aspect_ratio_threshold}, use_median={use_median}")
